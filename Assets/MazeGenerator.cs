    ...
    private int[,] cellToMap(int[,] cells)
    {
        ...
        for (int i = 0; i < numRows; i++)
            for (int j = 0; j < numCols; j++)
            {
                ...
                switch (cells[i, j]) {
                    case 0:
                        if (!hasWallMerged || UnityEngine.Random.Range(0f, 1f) <= probWallMerging) {//If hasn't merged before or prob proced, merge
                            if (j == numCols - 1) {//Check if right most column
    ...
    private void renderMap(int[,] map)
    {
        ...
        for (int i = 0; i < numRows; i++)
            for (int j = 0; j < numCols; j++) {
                ...
                TileBase usedTile = (map[i, j] != -1) ? tileBase : floorSprite;
                tilemap.SetTile(v, usedTile);
                if (j != 0) {
                    Vector3Int mirrorV = new Vector3Int (-j, i - yOffset, 0);
                    tilemap.SetTile(mirrorV, usedTile);
                }
            }
    }
